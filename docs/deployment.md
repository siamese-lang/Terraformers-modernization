...
현재 저장소는 Spring Boot backend, React production build, AWS runtime Terraform outputs, Kubernetes backend package, managed Secret 전달, private S3·CloudFront frontend delivery source contract, OIDC 기반 AWS workflow 경계를 검증한다. 실제 AWS apply, frontend object sync, CloudFront distribution 배포, backend load-balancer 연결, EKS rollout은 완료된 운영 상태로 간주하지 않는다.

## 2. 현재 구현 상태

| 영역 | 현재 저장소 상태 | 배포 전 남은 작업 |
|---|---|---|
| Frontend | React build, private S3/OAC/CloudFront, same-origin `/api/*`, build input bundle 정적 검증 | 실제 bucket/distribution 생성, S3 sync, invalidation, browser smoke |
| Backend | Spring Boot image build, local health, Kubernetes package 검증 | ECR publish 결과와 runtime manifest 연결, approved load-balancer origin, live rollout |
...
| Infrastructure | VPC/EKS/RDS/S3/SQS/ECR/Cognito/IAM/Secrets Manager/CloudFront source contract | 실제 state, plan, apply, output 값 검증 |
...
Backend runtime 입력은 다음 output을 사용한다.
...
| frontend delivery | `frontend_bucket_name`, `cloudfront_distribution_id`, `cloudfront_distribution_domain_name` |
...
Browser
  -> CloudFront HTTPS
       ├─ static route -> private versioned S3 bucket through OAC/SigV4
       └─ /api/*       -> approved HTTPS backend origin, caching disabled
```
...
생성기 `build-frontend-delivery-input-bundle.py`는 Cognito와 frontend Terraform output을 결합해 build env, delivery source map, S3 sync·CloudFront invalidation의 수동 순서를 만든다. AWS mutation은 수행하지 않는다.

현재 inventory에서 `frontend_bucket`과 `cloudfront_distribution` output group은 source 기준으로 해소된다. 다만 backend origin DNS와 load-balancer 경로가 정해지기 전에는 실제 CloudFront E2E가 완성되지 않는다.
...
9. CloudFront와 backend load-balancer origin 연결 및 직접 접근 경계
...
- frontend rollback과 cache invalidation 결과가 evidence로 남음
- 장애 시 원인 구간과 복구 상태를 evidence로 설명할 수 있음

현재 PR은 source/runtime/package, managed Secret, frontend delivery 정적 계약을 정리하는 단계이며, live AWS 검증은 별도 승인 단계다.
