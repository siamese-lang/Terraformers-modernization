...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/deploy/review-latest-stateful-dependencies-plan.sh \
    --expected-head CURRENT_SHA \
    --plan-head PLAN_SHA

Locate the successful stateful-dependencies plan for PLAN_SHA, download only its
sanitized artifact into a private directory, verify the exact five-resource
create set and safety gates, and print the review summary. CURRENT_SHA protects
the local review code. No AWS or GitHub mutation is performed.
EOF
}
...
if ! git diff --quiet "$PLAN_HEAD" "$ACTUAL_HEAD" -- infra/terraform/envs/backend-stateful-dependencies; then
  fail "STATEFUL_DEPENDENCIES_CONFIGURATION_CHANGED_SINCE_PLAN"
fi
...
RUN_RECORD="$(gh api \
  ...
  | head -n 1)"
[[ -n "$RUN_RECORD" ]] || fail "SUCCESSFUL_STATEFUL_DEPENDENCIES_PLAN_RUN_NOT_FOUND"
IFS=$'\t' read -r RUN_ID RUN_URL <<< "$RUN_RECORD"
[[ "$RUN_ID" =~ ^[0-9]+$ ]] || fail "STATEFUL_DEPENDENCIES_PLAN_RUN_ID_INVALID"
[[ -n "$RUN_URL" ]] || fail "STATEFUL_DEPENDENCIES_PLAN_RUN_URL_MISSING"

ARTIFACT_RECORD="$(gh api \
  "repos/${REPO}/actions/runs/${RUN_ID}/artifacts" \
  --jq ".artifacts[] | select(.name == \"${ARTIFACT_NAME}\" and .expired == false) | [.id, .name] | @tsv" \
  | head -n 1)"
[[ -n "$ARTIFACT_RECORD" ]] || fail "STATEFUL_DEPENDENCIES_PLAN_ARTIFACT_NOT_FOUND"
IFS=$'\t' read -r ARTIFACT_ID RESOLVED_ARTIFACT_NAME <<< "$ARTIFACT_RECORD"
[[ "$ARTIFACT_ID" =~ ^[0-9]+$ ]] || fail "STATEFUL_DEPENDENCIES_PLAN_ARTIFACT_ID_INVALID"
[[ "$RESOLVED_ARTIFACT_NAME" == "$ARTIFACT_NAME" ]] || fail "STATEFUL_DEPENDENCIES_PLAN_ARTIFACT_NAME_MISMATCH"
...
grep -Fqx 'terraform_plan_risk_gate=passed' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_PLAN_RISK_GATE_NOT_PASSED"
grep -Fqx 'plan_stage=stateful-dependencies' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_PLAN_STAGE_MISMATCH"
grep -Fqx 'resource_change_count=5' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_RESOURCE_COUNT_MISMATCH"
grep -Fqx 'destructive_resource_count=0' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_DESTRUCTIVE_CHANGE_PRESENT"
grep -Fqx 'replacement_resource_count=0' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_REPLACEMENT_PRESENT"
grep -Fqx 'public_exposure_finding_count=0' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_PUBLIC_EXPOSURE_PRESENT"
grep -Fqx 'optional_adapter_resource_count=0' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_OPTIONAL_ADAPTER_PRESENT"
grep -Fqx 'high_cost_resource_count=1' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_HIGH_COST_COUNT_MISMATCH"
grep -Fqx 'raw_plan_uploaded=false' "$RISK_TXT" || fail "STATEFUL_DEPENDENCIES_RAW_PLAN_BOUNDARY_MISSING"
...
if actual != expected:
    raise SystemExit("STATEFUL_DEPENDENCIES_RESOURCE_ACTION_SET_MISMATCH")
print("ExpectedResourceActionMatch=true")
print("ExpectedResourceCount=5")
PY
...
printf '%s\n' \
  "StatefulDependenciesPlanReview=passed" \
