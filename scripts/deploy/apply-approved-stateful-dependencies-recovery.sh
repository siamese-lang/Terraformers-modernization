...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/deploy/apply-approved-stateful-dependencies-recovery.sh \
    --expected-head CURRENT_SHA \
    --approved-plan-head PLAN_SHA

Complete the partial stateful-dependencies apply by requiring four existing
no-op resources and exactly one RDS create action. The command applies one
saved plan once, never runs terraform destroy, and never reads or prints the
RDS-managed database password value.
EOF
}
...
APPROVED_REVIEW_DIR="$PRIVATE_DIR/stateful-dependencies-recovery-plan-review-${APPROVED_SHORT}"
...
WORK_DIR="$PRIVATE_DIR/stateful-dependencies-recovery-apply-${EXPECTED_HEAD:0:12}"
TF_DATA_DIR_UNIX="$WORK_DIR/tfdata"
TF_CLI_CONFIG="$WORK_DIR/terraform.tfrc"
BACKEND_CONFIG="$WORK_DIR/backend.hcl"
PLAN_PATH="$WORK_DIR/stateful-dependencies-recovery.tfplan"
PLAN_JSON="$WORK_DIR/stateful-dependencies-recovery-plan.json"
PLAN_LOG="$WORK_DIR/stateful-dependencies-recovery-plan.log"
APPLY_LOG="$WORK_DIR/stateful-dependencies-recovery-apply.log"
POST_PLAN_PATH="$WORK_DIR/stateful-dependencies-recovery-post-apply.tfplan"
POST_PLAN_LOG="$WORK_DIR/stateful-dependencies-recovery-post-apply-plan.log"
PRE_STATE_LIST="$WORK_DIR/stateful-dependencies-pre-recovery-state-list.txt"
POST_STATE_LIST="$WORK_DIR/stateful-dependencies-post-recovery-state-list.txt"
...
RDS_PREFLIGHT_ERROR="$WORK_DIR/rds-preflight-error.log"
RISK_DIR="$WORK_DIR/risk"
SUMMARY_PATH="$WORK_DIR/stateful-dependencies-recovery-apply-summary.txt"
...
[[ -f "$APPROVED_RISK_MD" ]] || fail "APPROVED_STATEFUL_RECOVERY_PLAN_REVIEW_NOT_FOUND"
...
if ! git diff --quiet "$APPROVED_PLAN_HEAD" "$ACTUAL_HEAD" -- infra/terraform/envs/backend-stateful-dependencies; then
  fail "STATEFUL_DEPENDENCIES_CONFIGURATION_CHANGED_SINCE_APPROVED_RECOVERY_PLAN"
fi
...
grep -Fq 'database_allocated_storage_gb         = 20' "$STATEFUL_TFVARS" || fail "STATEFUL_RDS_STORAGE_DRIFT"
grep -Fq 'database_backup_retention_days        = 1' "$STATEFUL_TFVARS" || fail "STATEFUL_RDS_BACKUP_RETENTION_DRIFT"
...
"${PYTHON_CMD[@]}" - "$APPROVED_RISK_MD" <<'PY'
import re
import sys
...
if actual != expected:
    raise SystemExit("APPROVED_RECOVERY_RESOURCE_ACTION_SET_MISMATCH")
print("ApprovedRecoveryResourceActionMatch=true")
print("ApprovedRecoveryCreateCount=1")
print("ApprovedRecoveryNoOpCount=4")
PY
...
"$TF_EXE" -chdir="$WORK_DIR_WIN" state list > "$PRE_STATE_LIST"
PRE_MANAGED_STATE_COUNT="$(grep -v '^data\.' "$PRE_STATE_LIST" | grep -c . || true)"
[[ "$PRE_MANAGED_STATE_COUNT" == "4" ]] || fail "PRE_RECOVERY_MANAGED_STATE_COUNT_MISMATCH"
for address in \
  ...
  grep -Fqx "$address" "$PRE_STATE_LIST" || fail "PRE_RECOVERY_STATE_ADDRESS_MISSING"
done
if grep -Fqx 'aws_db_instance.backend' "$PRE_STATE_LIST"; then
  fail "PRE_RECOVERY_RDS_ALREADY_IN_STATE"
fi

set +e
aws rds describe-db-instances \
  --db-instance-identifier terraformers-modernization-dev-mariadb \
  --output json >/dev/null 2> "$RDS_PREFLIGHT_ERROR"
RDS_PREFLIGHT_EXIT_CODE=$?
set -e
if [[ "$RDS_PREFLIGHT_EXIT_CODE" -eq 0 ]]; then
  fail "PRE_RECOVERY_RDS_ALREADY_EXISTS"
fi
if ! grep -Eq 'DBInstanceNotFound|DBInstanceNotFoundFault' "$RDS_PREFLIGHT_ERROR"; then
  fail "PRE_RECOVERY_RDS_LOOKUP_FAILED"
fi
rm -f "$RDS_PREFLIGHT_ERROR"
...
PLAN_EXIT_CODE=$?
set -e
[[ "$PLAN_EXIT_CODE" -eq 0 ]] || fail "STATEFUL_RECOVERY_REPLAN_FAILED"
...
approved_text = Path(sys.argv[1]).read_text(encoding="utf-8")
plan = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
row_pattern = re.compile(r'^\| `([^`]+)` \| `([^`]+)` \| `([^`]+)` \|$', re.MULTILINE)
approved = {
    (address, resource_type, tuple(action for action in actions.split(",") if action))
    for address, resource_type, actions in row_pattern.findall(approved_text)
}
actual = {
    (
...
if approved != actual:
    raise SystemExit("RECOVERY_REPLAN_APPROVED_ACTION_MISMATCH")
if actual != expected:
    raise SystemExit("RECOVERY_REPLAN_EXPECTED_ACTION_SET_MISMATCH")
print("ApprovedRecoveryResourceActionMatch=true")
print("RecoveryCreateCount=1")
print("RecoveryNoOpCount=4")
PY

grep -Fqx 'terraform_plan_risk_gate=passed' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_RISK_GATE_FAILED"
grep -Fqx 'resource_change_count=5' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_RESOURCE_COUNT_MISMATCH"
grep -Fqx 'destructive_resource_count=0' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_DESTRUCTIVE_CHANGE_PRESENT"
grep -Fqx 'replacement_resource_count=0' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_REPLACEMENT_PRESENT"
grep -Fqx 'public_exposure_finding_count=0' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_PUBLIC_EXPOSURE_PRESENT"
grep -Fqx 'optional_adapter_resource_count=0' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_OPTIONAL_ADAPTER_PRESENT"
grep -Fqx 'high_cost_resource_count=1' "$RISK_DIR/plan-risk-summary.txt" || fail "RECOVERY_REPLAN_HIGH_COST_COUNT_MISMATCH"

sha256sum "$PLAN_PATH" > "$WORK_DIR/stateful-dependencies-recovery.tfplan.sha256"
printf '%s\n' \
  "StatefulDependenciesRecoveryApplyStarted=true" \
  "ApprovedPlanHead=${APPROVED_SHORT}" \
  "ApprovedRecoveryResourceActionMatch=true" \
...
if [[ "$APPLY_EXIT_CODE" -ne 0 ]]; then
  printf '%s\n' \
    "StatefulDependenciesRecoveryApplyStatus=failed-or-partial" \
...
"$TF_EXE" -chdir="$WORK_DIR_WIN" state list > "$POST_STATE_LIST"
POST_MANAGED_STATE_COUNT="$(grep -v '^data\.' "$POST_STATE_LIST" | grep -c . || true)"
[[ "$POST_MANAGED_STATE_COUNT" == "5" ]] || fail "POST_RECOVERY_MANAGED_STATE_COUNT_MISMATCH"
for address in \
  aws_cognito_user_pool.backend \
  aws_cognito_user_pool_client.backend \
  ...
  aws_db_subnet_group.backend \
  aws_security_group.backend_database; do
  grep -Fqx "$address" "$POST_STATE_LIST" || fail "POST_RECOVERY_STATE_ADDRESS_MISSING"
done
...
for name in required_strings:
    ...
    if not isinstance(value, str) or not value:
        raise SystemExit(f"STATEFUL_RECOVERY_OUTPUT_INVALID: {name}")
if outputs.get("database_port", {}).get("value") != 3306:
    raise SystemExit("STATEFUL_RECOVERY_DATABASE_PORT_INVALID")
if outputs["database_name"]["value"] != "terraformers":
    raise SystemExit("STATEFUL_RECOVERY_DATABASE_NAME_INVALID")
if outputs["cognito_region"]["value"] != "ap-northeast-2":
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_REGION_INVALID")
if not outputs["spring_datasource_url"]["value"].startswith("jdbc:mariadb://"):
    raise SystemExit("STATEFUL_RECOVERY_JDBC_URL_INVALID")
PY
...
[[ -n "$DB_INSTANCE_IDENTIFIER" && -n "$SG_ID" && -n "$POOL_ID" && -n "$CLIENT_ID" && -n "$MASTER_SECRET_ARN" ]] || fail "STATEFUL_RECOVERY_VERIFICATION_IDENTIFIERS_MISSING"
...
for key, expected in expected_rds.items():
    if rds.get(key) != expected:
        raise SystemExit(f"RDS_RECOVERY_PROPERTY_MISMATCH: {key}")
if not isinstance(rds.get("MasterUserSecret", {}).get("SecretArn"), str):
    raise SystemExit("RDS_RECOVERY_MANAGED_MASTER_SECRET_ARN_MISSING")

match = re.search(r'allowed_database_cidr_blocks\s*=\s*\[(.*?)\]', tfvars, re.DOTALL)
if not match:
    raise SystemExit("STATEFUL_RECOVERY_ALLOWED_DATABASE_CIDRS_NOT_FOUND")
expected_cidrs = set(re.findall(r'"([0-9.]+/[0-9]+)"', match.group(1)))
if len(expected_cidrs) != 2 or "0.0.0.0/0" in expected_cidrs:
    raise SystemExit("STATEFUL_RECOVERY_ALLOWED_DATABASE_CIDRS_INVALID")
actual_cidrs = set()
for permission in sg.get("IpPermissions", []):
    if permission.get("IpProtocol") != "tcp" or permission.get("FromPort") != 3306 or permission.get("ToPort") != 3306:
        raise SystemExit("STATEFUL_RECOVERY_DATABASE_INGRESS_UNEXPECTED")
    if permission.get("Ipv6Ranges") or permission.get("UserIdGroupPairs") or permission.get("PrefixListIds"):
        raise SystemExit("STATEFUL_RECOVERY_DATABASE_NON_CIDR_INGRESS_PRESENT")
    actual_cidrs.update(item.get("CidrIp") for item in permission.get("IpRanges", []))
if actual_cidrs != expected_cidrs:
    raise SystemExit("STATEFUL_RECOVERY_DATABASE_CIDR_MISMATCH")

if pool.get("Name") != "terraformers-modernization-dev-users":
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_POOL_NAME_MISMATCH")
if pool.get("DeletionProtection") != "INACTIVE" or pool.get("MfaConfiguration") != "OFF":
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_POOL_SETTING_MISMATCH")
if set(pool.get("UsernameAttributes", [])) != {"email"}:
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_USERNAME_ATTRIBUTE_MISMATCH")
if set(pool.get("AutoVerifiedAttributes", [])) != {"email"}:
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_AUTO_VERIFIED_ATTRIBUTE_MISMATCH")
if "ClientSecret" in client:
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_CLIENT_SECRET_UNEXPECTED")
...
if set(client.get("ExplicitAuthFlows", [])) != expected_flows:
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_AUTH_FLOW_MISMATCH")
if set(client.get("SupportedIdentityProviders", [])) != {"COGNITO"}:
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_PROVIDER_MISMATCH")
if client.get("PreventUserExistenceErrors") != "ENABLED":
    raise SystemExit("STATEFUL_RECOVERY_COGNITO_USER_EXISTENCE_BOUNDARY_MISMATCH")
PY
...
POST_PLAN_EXIT_CODE=$?
set -e
[[ "$POST_PLAN_EXIT_CODE" -eq 0 ]] || fail "POST_RECOVERY_PLAN_NOT_EMPTY"
...
[[ -n "$STATE_BUCKET" && -n "$STATE_KEY" ]] || fail "STATEFUL_RECOVERY_BACKEND_VALUES_MISSING"
if aws s3api head-object --bucket "$STATE_BUCKET" --key "${STATE_KEY}.tflock" >/dev/null 2>&1; then
  fail "STALE_STATEFUL_RECOVERY_LOCK_OBJECT_PRESENT"
fi
aws s3api head-object --bucket "$STATE_BUCKET" --key "$STATE_KEY" >/dev/null 2>&1 || fail "STATEFUL_RECOVERY_REMOTE_STATE_OBJECT_MISSING"

rm -f "$PLAN_JSON" "$PLAN_PATH" "$POST_PLAN_PATH"

cat > "$SUMMARY_PATH" <<EOF
StatefulDependenciesRecoveryApplyStatus=success
RepositoryHead=${ACTUAL_HEAD:0:12}
ApprovedPlanHead=${APPROVED_SHORT}
PythonCommand=${PYTHON_LABEL}
ProviderMemoryMode=GOGC-${GOGC}
ApprovedRecoveryResourceActionMatch=true
RecoveryCreatedResourceCount=1
RecoveryNoOpResourceCount=4
...
TerraformDestroyExecuted=false
GitHubMutation=none
AwsMutation=stateful-dependencies-rds-1-create
