...
function Dropzone({ onUploaded, onError }) {
  ...
  return <section className="dropzone-panel"><label className="upload-field">프로젝트 이름<input value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="Architecture project name" /></label><div {...getRootProps({ style })}><input {...getInputProps({ 'aria-label': 'PNG/JPEG architecture image' })} /><p>PNG/JPEG 아키텍처 이미지를 드래그하거나 클릭해 선택하세요.</p><p>시스템 구성요소와 연결 관계가 표시된 아키텍처 이미지를 업로드해 주세요.</p></div>{fileError && <p role="alert" className="error">{fileError}</p>}{files[0] && <div className="dropzone-preview"><img src={files[0].preview} alt={files[0].name} style={{ objectFit: 'contain', maxWidth: '100%', height: 'auto', maxHeight: 480 }} /><span>{files[0].name}</span></div>}<button type="button" className="primary-button" disabled={uploading} onClick={submit}>{uploading ? '분석 요청 중...' : '분석 시작'}</button></section>;
}
